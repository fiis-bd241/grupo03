...
<div>
<img src=".\resources\prototipo\V1-RFE.png" alt="I-153" style="width: 90%; height: auto;"/>
</div>
...
3. **Selección del nivel de acceso**
	...
	SELECT "Nivel de acceso" FROM "TipoDato"

	```

4. **Para las Opciones de Enmascarado (true y false que en el front serán transformados a Sí y No)**
...
4. **Para las Opciones de Encriptado (true y false que en el front serán transformados a Sí y No)**

	```sql

    SELECT "Encriptación" 
    FROM "TipoDato"
    GROUP BY "Encriptación";
...
5. **Botón agregar**
	* Cuando el usuario presione el botón agregar se llenará los campos con sus respectivos valores de seguridad.
	...
	INSERT INTO "Campo" ("ID_Modelo", "id_TipodeDato", "id_AlgoritmoEnc", "id_AlgoritmoEnm")
    VALUES (
    (SELECT "ID_Modelo" FROM "Modelado" WHERE "CampoDDV" = <2>),
    (SELECT "id_TipoDeDato" FROM "TipoDato" 
    WHERE "Nivel de acceso" = <3>
        AND "Encriptación" = <4>
        AND "Enmascarado" = <5>),
    null,
    null
    ...
    Donde <1> es el pedido seleccionado
...
<div>
<img src=".\resources\prototipo\V2-RFE.png" alt="I-153" style="width: 90%; height: auto;"/>
</div>
...
2. **Migracion según el pedido elegido**
...
3. **Botón agregar**
    ...
    ("Migracion_Id", "FrecuenciaEjecucion", "DiaInicio", "DiaFin", "ConsideracionFrecuencia")
    VALUES
    (<1>, <2>, <3>, <4>, <5>);
...
<div>
<img src=".\resources\prototipo\V3-RFE.png" alt="I-153" style="width: 90%; height: auto;"/>
</div>
...
4. **Botón registrar : se registrará los datos correspondientes al tipo de error**
    ...
    INSERT INTO "RegistroErrores" ("Migracion_Id", "Id_error", "Id_Empleado", "Correcion_error", "Fecha_registro", "Causa_error")
    VALUES (
    <1>,
    (SELECT "Id_error" FROM "TipoError" WHERE "Nombre_error" = <2>),
    (SELECT "id_empleado" FROM "Empleado" WHERE "nombre" = <3>),
    <4>,
    CURRENT_DATE,
    <5>
    );
	...
	Donde <1> es la Migracion_ID, <2> es el nombre del tipo de error seleccionado, <3> es el código de empleado , <4> es la corrección del error , <6> es la causa del error.
...
1. ** Seleccionar archivo de errores **
	*Al seleccionar el botón de archivo de errores, se mostrarán los errores registrados 
	...
	SELECT "Causa_error", "Correcion_error"
    FROM "RegistroErrores" re
